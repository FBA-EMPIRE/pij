...
function FormationDashboard({ lang }: { lang: "fr" | "en" }) {
  ...
  const completed = FORMATION_COURSES.filter((c) => c.progress === 100);

  return (
    <div className="p-4 lg:p-8 max-w-6xl mx-auto">
      <div className="mb-6 p-6 lg:p-8 rounded-2xl bg-card border border-border relative overflow-hidden">
        ...
        <div className="relative flex flex-col lg:flex-row lg:items-center lg:justify-between gap-5">
          <div>
            ...
            <h1 className="text-2xl font-bold mb-2" style={{ fontFamily: "DM Sans, sans-serif" }}>{fr ? "Apprendre, structurer, puis grandir" : "Learn, structure, then grow"}</h1>
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            <button onClick={() => navigate("/formations/courses/course-entreprendre")} className="flex items-center justify-center gap-2 px-5 py-3 rounded-xl text-white font-medium text-sm hover:opacity-90" style={{ background: "linear-gradient(135deg, #4CAF68, #1F9D55)" }}><Play size={16} />{fr ? "Continuer" : "Continue"}</button>
            <button onClick={() => navigate("/formations/consultation")} className="flex items-center justify-center gap-2 px-5 py-3 rounded-xl border border-border text-sm font-medium hover:bg-muted"><Users size={16} />{fr ? "Demander conseil" : "Request guidance"}</button>
          </div>
        </div>
      </div>

      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
        {[{ label: fr ? "En cours" : "In progress", value: inProgress.length, icon: Play }, { label: fr ? "Terminés" : "Completed", value: completed.length, icon: Award }, { label: fr ? "Progression moyenne" : "Average progress", value: "48%", icon: GraduationCap }].map((s) => (
          <div key={s.label} className="bg-card rounded-2xl border border-border p-5">
            <div className="flex items-center justify-between mb-2"><p className="text-sm text-muted-foreground">{s.label}</p><s.icon size={18} color="#4CAF68" /></div>
            <p className="text-2xl font-bold" style={{ fontFamily: "Geist Mono, monospace" }}>{s.value}</p>
          </div>
        ))}
      </div>
      ...
      {inProgress[0] && (
        <div className="bg-card rounded-2xl border border-border p-5 mb-8">
          <div className="flex items-center justify-between mb-4"><h2 className="text-lg font-bold" style={{ fontFamily: "DM Sans, sans-serif" }}>{fr ? "Continuer l'apprentissage" : "Continue learning"}</h2><button onClick={() => navigate("/formations/learning")} className="text-xs text-[#4CAF68] font-medium flex items-center gap-1 hover:underline">{fr ? "Mon apprentissage" : "My learning"}<ChevronRight size={12} /></button></div>
      ...
      <section className="mb-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {FORMATION_CATEGORIES.map((cat) => (
            <button key={cat.id} onClick={() => navigate(`/formations/courses/${FORMATION_COURSES.find((c) => c.categoryId === cat.id)?.id ?? "course-entreprendre"}`)} className="bg-card rounded-2xl border border-border p-5 text-left hover:border-[#4CAF68]/40 transition-all">
...
function CourseCard({ course, lang }: { course: typeof FORMATION_COURSES[number]; lang: "fr" | "en" }) {
  const fr = lang === "fr";
  const navigate = useNavigate();
  return <div className="bg-card rounded-2xl border border-border overflow-hidden group hover:border-[#4CAF68]/40 transition-all"><div className="relative h-40" style={{ background: course.image }}><div className="absolute inset-0 bg-black/20 group-hover:bg-black/10" /><span className="absolute bottom-3 right-3 flex items-center gap-1 px-2 py-1 rounded-md bg-black/60 text-white text-xs"><Clock size={12} />{course.duration}</span></div><div className="p-4"><h3 className="text-sm font-semibold mb-1" style={{ fontFamily: "DM Sans, sans-serif" }}>{fr ? course.title : course.titleEn}</h3><p className="text-xs text-muted-foreground leading-relaxed mb-3">{course.description}</p><div className="flex items-center justify-between text-xs text-muted-foreground mb-2"><span>{course.lessonCount} {fr ? "leçons" : "lessons"}</span><span className="text-[#4CAF68] font-bold">{course.progress}%</span></div><div className="w-full bg-muted rounded-full h-2 mb-4"><div className="h-2 rounded-full bg-[#4CAF68]" style={{ width: `${course.progress}%` }} /></div><button onClick={() => navigate(`/formations/courses/${course.id}`)} className="w-full py-2.5 rounded-xl text-white text-sm font-medium hover:opacity-90" style={{ background: "#4CAF68" }}>{fr ? "Voir le cours" : "View course"}</button></div></div>;
}
...
function CourseDetail({ lang }: { lang: "fr" | "en" }) {
  ...
  const course = FORMATION_COURSES.find((c) => c.id === id) ?? FORMATION_COURSES[0];
  const items = FORMATION_CONTENT.filter((c) => c.courseId === course.id);
  ...
  return <div className="p-4 lg:p-8 max-w-5xl mx-auto"><button onClick={() => navigate("/formations")} className="text-sm text-[#4CAF68] mb-4 hover:underline">← {fr ? "Retour aux formations" : "Back to formations"}</button><div className="bg-card rounded-2xl border border-border overflow-hidden mb-6"><div className="h-48" style={{ background: course.image }} /><div className="p-6"><div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4"><div><h1 className="text-2xl font-bold" style={{ fontFamily: "DM Sans, sans-serif" }}>{fr ? course.title : course.titleEn}</h1><p className="text-sm text-muted-foreground mt-2 max-w-2xl">{course.description}</p></div><button onClick={() => navigate("/formations/consultation")} className="px-4 py-2.5 rounded-xl border border-border text-sm font-medium hover:bg-muted">{fr ? "Demander une consultation" : "Request consultation"}</button></div><div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mt-5 text-sm"><Stat label={fr ? "Durée" : "Duration"} value={course.duration} /><Stat label={fr ? "Leçons" : "Lessons"} value={`${course.lessonCount}`} /><Stat label={fr ? "Niveau" : "Level"} value={course.level} /><Stat label={fr ? "Progression" : "Progress"} value={`${course.progress}%`} /></div></div></div><div className="flex gap-2 mb-5 overflow-x-auto">{tabs.map((t) => <button key={t.key} onClick={() => setTab(t.key)} className={`px-4 py-2 rounded-xl text-sm font-medium border whitespace-nowrap ${tab === t.key ? "bg-[#4CAF68] text-white border-[#4CAF68]" : "bg-card border-border text-muted-foreground"}`}>{t.label}</button>)}</div>{tab === "overview" ? <div className="bg-card rounded-2xl border border-border p-6"><h3 style={{ fontFamily: "DM Sans, sans-serif" }}>{fr ? "Ce que vous allez apprendre" : "What you will learn"}</h3><div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mt-4">{[fr ? "Valider une idée terrain" : "Validate a field idea", fr ? "Structurer un modèle économique" : "Structure a business model", fr ? "Préparer une demande de financement" : "Prepare a funding request", fr ? "Passer à l'action avec un plan clair" : "Move forward with a clear plan"].map((x) => <div key={x} className="flex gap-2 text-sm text-muted-foreground"><CheckCircle size={16} color="#4CAF68" />{x}</div>)}</div></div> : <ContentList items={items.filter((i) => i.type === tab)} fr={fr} />}</div>;
}

function ContentList({ items, fr }: { items: typeof FORMATION_CONTENT; fr: boolean }) {
...
function MyLearning({ lang }: { lang: "fr" | "en" }) {
  const fr = lang === "fr";
  return <div className="p-4 lg:p-8 max-w-5xl mx-auto"><h1 className="text-2xl font-bold mb-1" style={{ fontFamily: "DM Sans, sans-serif" }}>{fr ? "Mon apprentissage" : "My learning"}</h1><p className="text-sm text-muted-foreground mb-6">{fr ? "Suivez vos cours terminés et en cours." : "Track completed and in-progress courses."}</p><div className="space-y-6"><section className="bg-card rounded-2xl border border-border p-5"><h2 className="text-lg font-bold mb-4">{fr ? "En cours" : "In progress"}</h2><div className="space-y-4">{FORMATION_COURSES.filter((c) => c.progress < 100).map((c) => <CourseRow key={c.id} course={c} lang={lang} />)}</div></section><section className="bg-card rounded-2xl border border-border p-5"><h2 className="text-lg font-bold mb-4">{fr ? "Terminés" : "Completed"}</h2>{FORMATION_COURSES.filter((c) => c.progress === 100).map((c) => <CourseRow key={c.id} course={c} lang={lang} />)}</section></div></div>;
}

function ConsultationRequest({ lang }: { lang: "fr" | "en" }) {
  const fr = lang === "fr";
  const [type, setType] = useState(consultationTypes[0]);
  return <div className="p-4 lg:p-8 max-w-5xl mx-auto"><h1 className="text-2xl font-bold mb-1" style={{ fontFamily: "DM Sans, sans-serif" }}>{fr ? "Demande de consultation" : "Consultation request"}</h1><p className="text-sm text-muted-foreground mb-6">{fr ? "Mentorat, consultation, revue business ou évaluation de projet." : "Mentorship, consultation, business review or project evaluation."}</p><div className="grid grid-cols-1 lg:grid-cols-2 gap-6"><div className="bg-card rounded-2xl border border-border p-6 space-y-4"><div><label className="text-sm font-medium">{fr ? "Type" : "Type"}</label><select value={type} onChange={(e) => setType(e.target.value)} className="mt-1.5 w-full px-3 py-2.5 rounded-xl border border-border bg-input-background text-sm focus:outline-none focus:ring-2 focus:ring-[#4CAF68]/40">{consultationTypes.map((t) => <option key={t}>{t}</option>)}</select></div><div><label className="text-sm font-medium">{fr ? "Projet" : "Project"}</label><input className="mt-1.5 w-full px-3 py-2.5 rounded-xl border border-border bg-input-background text-sm focus:outline-none focus:ring-2 focus:ring-[#4CAF68]/40" placeholder={fr ? "Nom de votre projet" : "Project name"} /></div><div><label className="text-sm font-medium">{fr ? "Besoin" : "Need"}</label><textarea rows={4} className="mt-1.5 w-full px-3 py-2.5 rounded-xl border border-border bg-input-background text-sm focus:outline-none focus:ring-2 focus:ring-[#4CAF68]/40 resize-none" placeholder={fr ? "Décrivez votre besoin..." : "Describe your need..."} /></div><button className="w-full flex items-center justify-center gap-2 px-5 py-3 rounded-xl text-white font-medium text-sm" style={{ background: "linear-gradient(135deg, #1E2530, #2A3444)" }}><Send size={16} />{fr ? "Envoyer la demande" : "Send request"}</button></div><div className="bg-card rounded-2xl border border-border p-6"><h2 className="text-lg font-bold mb-4" style={{ fontFamily: "DM Sans, sans-serif" }}>{fr ? "Suivi des demandes" : "Request tracking"}</h2><div className="space-y-3">{CONSULTATION_REQUESTS.map((r) => <div key={r.id} className="p-4 rounded-xl bg-muted/30"><div className="flex items-center justify-between gap-3"><p className="text-sm font-medium">{r.type}</p><StatusBadge status={r.status as any} size="sm" /></div><p className="text-xs text-muted-foreground mt-1">{r.project} · {r.consultant}</p><p className="text-xs text-muted-foreground mt-1">{r.meetingDate}</p></div>)}</div></div></div></div>;
}
