...
import { getCurrentUserId, createInvestmentRequest } from "../lib/supabase/queries";
    ...
    (async () => {
      try {
        ...
        const [opps, port, reqs, { data: profile }] = await Promise.all([
          supabase.from("investment_opportunities").select("*").eq("status", "Published"),
          supabase.from("investment_portfolio").select("*, investment_opportunities(title)").eq("user_id", userId),
          supabase.from("investment_requests").select("*, investment_opportunities(title)").eq("user_id", userId),
          supabase.from("users").select("balance_current, balance_investment").eq("id", userId).single(),
        ]);
        setOpportunities(opps.data ?? []);
        setPortfolio((port.data ?? []).map((p: any) => ({ ...p, title: p.investment_opportunities?.title ?? "—" })));
        setRequests((reqs.data ?? []).map((r: any) => ({ ...r, opportunity: r.investment_opportunities?.title ?? "—" })));
        setWalletData({
