...
type CourseContent = typeof FORMATION_CONTENT[number] & { fileName?: string; fileSize?: string };
type Consultation = typeof CONSULTATION_REQUESTS[number];
...
export default function AdminFormations({ lang = "fr" }: AdminFormationsProps) {
  ...
  const [categories, setCategories] = useState<Category[]>(FORMATION_CATEGORIES);
  const [courses, setCourses] = useState<Course[]>(FORMATION_COURSES);
  const [contents, setContents] = useState<CourseContent[]>(FORMATION_CONTENT);
  const [consultations, setConsultations] = useState<Consultation[]>(CONSULTATION_REQUESTS);
  ...
  return (
    <div className="p-4 lg:p-6">
      ...
      {formMode && tab === "categories" && (
        <CategoryForm fr={fr} mode={formMode} category={categories.find((c) => c.id === editingId)} onCancel={closeForm} onSave={(category) => {
      ...
      {formMode && tab === "courses" && (
        <CourseForm fr={fr} mode={formMode} course={courses.find((c) => c.id === editingId)} categories={categories} onCancel={closeForm} onSave={(course) => {
      ...
      {formMode && tab === "content" && (
        <ContentForm fr={fr} mode={formMode} item={contents.find((c) => c.id === editingId)} courses={courses} onCancel={closeForm} onSave={(item) => {
      ...
      {formMode && tab === "consultations" && (
        <ConsultationForm fr={fr} consultation={consultations.find((c) => c.id === editingId)} onCancel={closeForm} onSave={(consultation) => {
          setConsultations((current) => current.map((c) => c.id === consultation.id ? consultation : c));
          closeForm();
        }} />
      )}

      {tab === "categories" && <Categories fr={fr} categories={categories} onEdit={openEdit} onDelete={(id) => setCategories((current) => current.filter((c) => c.id !== id))} />}
      {tab === "courses" && <Courses fr={fr} courses={courses} onEdit={openEdit} onArchive={(id) => setCourses((current) => current.map((c) => c.id === id ? { ...c, status: "Archived" } : c))} />}
      {tab === "content" && <Content fr={fr} contents={contents} onEdit={openEdit} />}
      {tab === "consultations" && <Consultations fr={fr} consultations={consultations} onEdit={openEdit} onClose={(id) => setConsultations((current) => current.map((c) => c.id === id ? { ...c, status: "Closed" } : c))} />}
    </div>
  );
}

function Categories({ fr, categories, onEdit, onDelete }: { fr: boolean; categories: Category[]; onEdit: (id: string) => void; onDelete: (id: string) => void }) {
  return <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">{categories.map((cat) => <div key={cat.id} className="bg-card rounded-2xl border border-border p-5"><div className="w-10 h-10 rounded-xl flex items-center justify-center mb-3" style={{ background: `${cat.color}20` }}><BookOpen size={18} color={cat.color} /></div><div className="flex items-center justify-between gap-2"><h3 className="text-sm font-semibold">{cat.name}</h3><StatusBadge status={cat.status as any} size="sm" /></div><p className="text-xs text-muted-foreground mt-2">{cat.description}</p><div className="flex gap-2 mt-4"><button onClick={() => onEdit(cat.id)} className="px-3 py-1.5 rounded-lg border border-border text-xs">{fr ? "Modifier" : "Edit"}</button><button onClick={() => onDelete(cat.id)} className="px-3 py-1.5 rounded-lg border border-border text-xs text-[#E5484D]">{fr ? "Supprimer" : "Delete"}</button></div></div>)}</div>;
}

function Courses({ fr, courses, onEdit, onArchive }: { fr: boolean; courses: Course[]; onEdit: (id: string) => void; onArchive: (id: string) => void }) {
  return <div className="space-y-4">{courses.map((course) => <div key={course.id} className="bg-card rounded-2xl border border-border p-5"><div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4"><div><div className="flex items-center gap-2 mb-1"><h3 style={{ fontFamily: "DM Sans, sans-serif", fontWeight: 600 }}>{course.title}</h3><StatusBadge status={course.status as any} size="sm" /></div><p className="text-xs text-muted-foreground">{course.lessonCount} {fr ? "leçons" : "lessons"} · {course.duration} · {course.instructor}</p><p className="text-sm text-muted-foreground mt-3 max-w-2xl">{course.description}</p></div><div className="flex gap-2"><button onClick={() => onEdit(course.id)} className="px-3 py-1.5 rounded-lg border border-border text-xs">{fr ? "Modifier" : "Edit"}</button><button onClick={() => onArchive(course.id)} className="flex items-center gap-1 px-3 py-1.5 rounded-lg border border-border text-xs text-muted-foreground"><Archive size={13} />{fr ? "Archiver" : "Archive"}</button></div></div></div>)}</div>;
}

function Content({ fr, contents, onEdit }: { fr: boolean; contents: CourseContent[]; onEdit: (id: string) => void }) {
  const icon = (type: string) => type === "video" ? Video : FileText;
  return <div className="bg-card rounded-2xl border border-border p-5"><h3 className="mb-4" style={{ fontFamily: "DM Sans, sans-serif", fontWeight: 600 }}>{fr ? "Contenus liés aux cours" : "Course content"}</h3><div className="space-y-2">{contents.map((item) => { const Icon = icon(item.type); return <div key={item.id} className="flex items-center gap-3 p-3 rounded-xl bg-muted/30"><div className="w-9 h-9 rounded-xl bg-[#E8F5EC] flex items-center justify-center"><Icon size={16} color="#4CAF68" /></div><div className="flex-1 min-w-0"><p className="text-sm font-medium truncate">{item.title}</p><p className="text-xs text-muted-foreground">{item.format} · {item.duration}</p>{item.fileName && <p className="text-xs text-[#4CAF68] truncate mt-0.5">{item.fileName}{item.fileSize ? ` · ${item.fileSize}` : ""}</p>}</div><button onClick={() => onEdit(item.id)} className="px-3 py-1.5 rounded-lg border border-border text-xs">{fr ? "Gérer" : "Manage"}</button></div>; })}</div></div>;
}

function Consultations({ fr, consultations, onEdit, onClose }: { fr: boolean; consultations: Consultation[]; onEdit: (id: string) => void; onClose: (id: string) => void }) {
  return <div className="space-y-4">{consultations.map((request) => <div key={request.id} className="bg-card rounded-2xl border border-border p-5"><div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4"><div><div className="flex items-center gap-2 mb-1"><h3 className="text-sm font-semibold">{request.type}</h3><StatusBadge status={request.status as any} size="sm" /></div><p className="text-xs text-muted-foreground">{request.member} · {request.project}</p><p className="text-xs text-muted-foreground mt-1">{request.consultant} · {request.meetingDate}</p></div><div className="flex flex-wrap gap-2"><button onClick={() => onEdit(request.id)} className="px-3 py-1.5 rounded-lg border border-border text-xs">{fr ? "Assigner" : "Assign"}</button><button onClick={() => onEdit(request.id)} className="flex items-center gap-1 px-3 py-1.5 rounded-lg border border-border text-xs"><Calendar size={13} />{fr ? "Planifier" : "Schedule"}</button><button onClick={() => onClose(request.id)} className="px-3 py-1.5 rounded-lg text-white text-xs" style={{ background: "#4CAF68" }}>{fr ? "Clôturer" : "Close"}</button></div></div></div>)}</div>;
}

function CategoryForm({ fr, mode, category, onSave, onCancel }: { fr: boolean; mode: Exclude<FormMode, null>; category?: Category; onSave: (category: Category) => void; onCancel: () => void }) {
